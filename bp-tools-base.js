...
(function () {
  ...
  class InfoRegionGroup extends HTMLElement {
    ...
    _setupStepCollapse() {
      ...
      // MutationObserver：新步驟展開時，折疊前面已完成的步驟
      const obs = new MutationObserver(() => {
        const activeIdx = children.findIndex(el => el.getAttribute('active') === 'true'
          && !el.classList.contains('ir-step-collapsed')
          && !el.classList.contains('ir-step-expanded'));

        if (activeIdx === -1) return;

        children.forEach((el, i) => {
          if (i < activeIdx && el.getAttribute('active') === 'true') {
            // 前面已完成的步驟：折疊
            if (!el.classList.contains('ir-step-collapsed') && !el.classList.contains('ir-step-expanded')) {
              el.stepCollapse();
            }
          }
        });
