...
(function () {
  ...
  function injectStyles() {
    ...
    const css = `
      ...
      .ir-step-bar-title {
        flex: 1; font-size: .88rem; font-weight: 600; letter-spacing: .02em;
      }
      .ir-step-bar-reread {
        font-size: .7rem; opacity: .6; white-space: nowrap;
        border: 1px solid currentColor; border-radius: 4px;
        padding: 2px 8px; flex-shrink: 0;
        transition: opacity .2s;
      }
      .ir-step-bar:hover .ir-step-bar-reread { opacity: 1; }
      /* 折疊態：max-height 限制為 bar 高度（44px = 11*2 + line-height）*/
      info-region.ir-step-collapsed {
      ...
      info-region.ir-step-collapsed .ir-step-bar { display: flex; }
      info-region.ir-step-collapsed .ir-step-main { display: none; }
      info-region.ir-step-collapsed .ir-manual-wrap { display: none; }
      info-region.ir-step-collapsed .ir-countdown-bar { display: none; }
      /* 展開中（re-read）*/
      info-region.ir-step-expanded {
        max-height: 4000px; opacity: 1; transform: translateY(0);
        pointer-events: auto;
      }
      info-region.ir-step-expanded .ir-step-bar { display: flex; }
      info-region.ir-step-expanded .ir-step-main { display: block; }
    `;
  ...
  class InfoRegion extends HTMLElement {
    ...
    reset() {
      ...
      // 重設折疊狀態
      this.classList.remove('ir-step-collapsed', 'ir-step-expanded');
      const main = this.querySelector('.ir-step-main');
    ...
    /**
     * 初始化折疊條（由 InfoRegionGroup._setupStepCollapse() 呼叫）
     * @param {number} stepNum  步驟編號（1-based）
     * @param {string} title    折疊後顯示的標題
     * @param {string} colorHex 品牌色（hex）
     * @param {boolean} reread  是否顯示「重新閱讀」提示
     */
    _initStepBar(stepNum, title, colorHex, reread) {
      if (this.querySelector('.ir-step-bar')) return; // 已初始化
      // 把現有子內容包進 .ir-step-main
      const main = document.createElement('div');
      main.className = 'ir-step-main';
      Array.from(this.childNodes).forEach(n => main.appendChild(n));
      this.appendChild(main);
      // 建立折疊條
      const bar = document.createElement('div');
      bar.className = 'ir-step-bar';
      bar.style.color = colorHex || 'var(--color-sky, #04b5a3)';
      // ✓ 圓形
      const chk = document.createElement('span');
      chk.className = 'ir-step-bar-check';
      chk.style.background = (colorHex || '#04b5a3') + '22';
      chk.style.color       = colorHex || 'var(--color-sky, #04b5a3)';
      chk.style.border      = `1px solid ${colorHex || '#04b5a3'}55`;
      chk.innerHTML = `<span style="font-size:.75rem;">✓</span>`;
      // 標題
      const ttl = document.createElement('span');
      ttl.className   = 'ir-step-bar-title';
      ttl.textContent = title || `步驟 ${stepNum}`;
      // 步驟編號 badge
      const num = document.createElement('span');
      num.style.cssText = `font-size:.68rem;opacity:.55;font-family:monospace;`;
      num.textContent   = `Step ${stepNum}`;
      bar.append(chk, num, ttl);
      if (reread) {
        const rr = document.createElement('span');
        rr.className   = 'ir-step-bar-reread';
        rr.textContent = '重新閱讀 ↕';
        bar.appendChild(rr);
      }
      // 點擊展開 / 折疊切換
      bar.addEventListener('click', () => {
        if (this.classList.contains('ir-step-expanded')) {
          this.stepCollapse();
        } else {
          this.stepExpand();
        }
    ...
    /** 折疊為標題列 */
    stepCollapse() {
      this.classList.remove('ir-step-expanded');
      this.classList.add('ir-step-collapsed');
      // 清除可能被 _onActivated 設定的行內 opacity/transform，讓 CSS class 接管
      this.style.opacity   = '';
    ...
    /** 展開顯示完整內容（re-read） */
    stepExpand() {
      this.classList.remove('ir-step-collapsed');
      this.classList.add('ir-step-expanded');
      const cfg = window.InfoRegionConfig;
      this.style.background = cfg.bgColor;
      this.style.color      = cfg.textColor;
      this.style.padding    = cfg.padding;
      this._applyBorderStyles();
    }
  }
  ...
  class InfoRegionGroup extends HTMLElement {
    ...
    _build() {
      ...
      if (this.hasAttribute('step-collapse'))    { this._setupStepCollapse(); }
    ...
    /* ── step-collapse ───────────────────────────────────────── */
    /**
     * 啟用「前面步驟折疊、當前步驟完整展開」行為。
     *
     * 屬性（在 info-region-group 上設定）：
     *   step-collapse                  ← 啟用此功能
     *   collapse-reread                ← 顯示「重新閱讀」提示
     *
     * 屬性（在各 info-region 上設定）：
     *   collapsible-title="規則一：…"  ← 折疊後顯示的標題文字
     */
    _setupStepCollapse() {
      const children = Array.from(this.querySelectorAll(':scope > info-region'));
      if (!children.length) return;
      const reread = this.hasAttribute('collapse-reread');

      // 為每個 info-region 初始化折疊條
      children.forEach((child, i) => {
        const stepNum   = i + 1;
        const title     = child.getAttribute('collapsible-title') || `步驟 ${stepNum}`;
        const colorName = child.getAttribute('color') || 'sky';
        const colorHex  = (typeof BrandColors !== 'undefined')
          ? BrandColors.get(colorName) || '#04b5a3'
          : '#04b5a3';
        child._initStepBar(stepNum, title, colorHex, reread);
      });

      // MutationObserver：從 mutation.target 直接取得「剛變成 active 的元素」
      // 然後折疊它之前所有已完成的步驟
      const obs = new MutationObserver((mutations) => {
        for (const m of mutations) {
          const activated = m.target;
          // 只處理「剛剛被設為 active="true"」的情況
          if (activated.getAttribute('active') !== 'true') continue;
          // 若已是折疊 / 重讀展開狀態，跳過（避免 re-expand 時誤觸）
          if (activated.classList.contains('ir-step-collapsed')
              || activated.classList.contains('ir-step-expanded')) continue;

          const activatedIdx = children.indexOf(activated);
          if (activatedIdx <= 0) continue;   // 是第一步，前面沒有需要折疊的

          // 折疊所有在它之前、已 active 且尚未折疊的步驟
          for (let i = 0; i < activatedIdx; i++) {
            const prev = children[i];
            if (prev.getAttribute('active') === 'true'
                && !prev.classList.contains('ir-step-collapsed')
                && !prev.classList.contains('ir-step-expanded')) {
              prev.stepCollapse();
            }
          }
        }
      ...
      children.forEach(child => {
        obs.observe(child, { attributes: true, attributeFilter: ['active'] });
      });

      this._stepCollapseObs = obs;
    }
  }
