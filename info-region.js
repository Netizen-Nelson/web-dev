(function () {
  ...
  class InfoRegionGroup extends HTMLElement {
    ...
    /*
     * 每列顯示數量由以下優先順序決定（高→低）：
     *   元素屬性 cols-per-row  >  InfoRegionConfig.colsPerRow  >  預設 4
     *
     * 用法範例：
     *   <info-region-group layout="horizontal" cols-per-row="3">
     *
     * 步驟超過 cols-per-row 時自動折行，每 N 個步驟建立一列 Bootstrap row。
     * 列間距由 row-gap 屬性控制（預設 "16px"），欄間距沿用 gutter 屬性。
     */
    _setupLayout() {
      ...
      // 切成每 perRow 個一組，各自產生一個 Bootstrap row
    ...
    /*
     * 依每列欄數自動換算 Bootstrap 欄位 class。
     * 參數改為 perRow（每列欄數）而非步驟總數。
     */
