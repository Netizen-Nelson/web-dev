...
(function (G) {
    ...
    function injectCSS() {
        ...
        _cssEl.textContent = [
            ...
            /* 單格 input */
            '.bpi-digit{',
            'width:' + t.digitWidth + ';height:' + t.digitHeight + ';',
            'text-align:center;font-size:' + t.digitFontSize + ';font-weight:700;',
    ...
    BpInviteWidget.prototype._parse = function () {
        ...
        // 格子尺寸屬性（有設定才覆蓋全域 Config，使 injectCSS 即時生效）
        var dw = el.getAttribute('digit-width');
        var dh = el.getAttribute('digit-height');
        var df = el.getAttribute('digit-font-size');
        if (dw) Config.theme.digitWidth    = dw;
        if (dh) Config.theme.digitHeight   = dh;
        if (df) Config.theme.digitFontSize = df;
